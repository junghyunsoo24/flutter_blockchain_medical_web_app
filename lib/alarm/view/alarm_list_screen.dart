...
class _AlarmListScreenState extends State<AlarmListScreen> {
  ...
  void _handleNotificationTap(NotificationResponse response) {
    final alarmId = response.id;
    final alarm = alarms.firstWhere((a) => a.id == alarmId);
    if (alarm != null) {
      _viewModel.confirmTakeMedication(alarm);
  ...
  Future<void> _loadAlarms() async {
    final alarmList = await GetIt.I<MyDatabase>().getAllAlarms();
    setState(() {
      alarms = alarmList;
    });
  }
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => AlarmMedicalHistoryScreen()),
              );
            },
          ),
        ],
